...
export function Footer() {
  return (
    <footer className="section-dark relative overflow-hidden border-t border-white/10 pb-8 pt-24 sm:pt-32">
      <div
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="xl:grid xl:grid-cols-3 xl:gap-8">
          <div className="space-y-8 xl:col-span-1">
            <div className="flex items-center gap-3">
              <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-white text-sm font-bold tracking-tight text-black shadow-lg ring-1 ring-white/20 transition-transform hover:scale-105">
                V
              </span>
              <div>
            ...
            <form className="relative max-w-xs pt-2">
              <input
                ...
                className="w-full rounded-full border border-white/10 bg-white py-2.5 pl-4 pr-12 text-sm text-zinc-900 transition-colors placeholder:text-zinc-400 focus:border-emerald-500/50 focus:bg-white focus:outline-none focus:ring-1 focus:ring-emerald-500/50"
              />
              <button
                type="button"
                className="absolute right-1 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full bg-zinc-200 text-zinc-500 transition-colors hover:bg-emerald-500 hover:text-white"
              >
            ...
            <div className="flex gap-x-5 pt-4">
              {footerLinks.socials.map((item) => (
                <a key={item.name} href={item.href} className="text-zinc-400 transition-colors hover:text-zinc-900">
        ...
        <div className="mt-16 flex flex-col items-center justify-between border-t border-white/10 pt-8 sm:mt-20 md:flex-row">
          ...
          <div className="mt-4 flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 md:mt-0">
            <span className="relative flex h-2 w-2">
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-75"></span>
              <span className="relative inline-flex h-2 w-2 rounded-full bg-emerald-500"></span>
            </span>
            <span className="text-xs font-medium text-zinc-500">Profile details updated</span>
          </div>
        </div>
      </div>
    </footer>
